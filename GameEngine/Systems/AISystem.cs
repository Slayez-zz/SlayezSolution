            ...
            foreach (GameObject obj in SystemManager.gameObjects)
                if (obj.HasComponentType(typeof (BasicAI)))
                    if (obj.Component<BasicAI>().HasTarget)
                    {
                        ...
                        {
                            if (Utilites.Distance(obj.Component<Components.Main.Transform>().Position, obj.Component<BasicAI>().TargetPoint()) < objBody.Width / 2)
                                obj.Component<Components.Main.Transform>().Position = obj.Component<BasicAI>().TargetPoint();
